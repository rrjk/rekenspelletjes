...
export class NumberLineHangingPhotos extends LitElement {
    ...
    translatePosition(position: number): number{
        const relativePositionInNumberLineUnits = (position-this.minimum) / (this.maximum-this.minimum)
        const positionInSvgUnits = (relativePositionInNumberLineUnits * this.lineWidth) + this.margin - 1; // 1 substracted as we have lines of 2 wide.
        const relativePositionInSvgUnits = positionInSvgUnits / (this.lineWidth + 2*this.margin);
        const positionInVwUnits = relativePositionInSvgUnits * 0.95 * this.width + 0.025 * this.width;
    ...
    translateVerticalDistance(verticalDistance: verticalDistanceEnum): verticalDisctanceInfoType {
        ...
        if (verticalDistance === 'low'){
            ...
            verticalDistanceInfo.lineHeight = this.width*0.030;
            verticalDistanceInfo.viewBoxHeight = 0.030/0.05 * 51;
        }
    ...
    renderLine(position: number, verticalDistance:verticalDistanceEnum, lineColor: string): SVGTemplateResult {
        return svg`
            <svg style="position:absolute; 
                        ...
                        height: ${this.translateVerticalDistance(verticalDistance).lineHeight}vw; 
                        left: ${this.translatePosition(position)}vw; 
                        top: 0;" 
                viewBox="0 0 51 ${this.translateVerticalDistance(verticalDistance).viewBoxHeight}">
                <line
                x1 = "1"
                x2 = "1"
                y1 = "0"
                y2 = "${this.translateVerticalDistance(verticalDistance).viewBoxHeight}"
    ...
    render(): HTMLTemplateResult{
      return html`
          ...
          ${this.photoMetaData.map(metaData => this.renderFramedPhoto(metaData.position, metaData.verticalDistance, metaData.photoId))}
      `
    } 
};
