...
//        this.startNewGame();
    ...
    async showWelcomeMessage(){
        return this._messageDialog.show('De juiste foto kiezen', html `<p>Kies de juiste foto op de getallenlijn.</p><p>Dit spel kun je op de telefoon het beste horizontaal spelen.</p>`);
    }
    ...
    handleTimeUp(): void{
        ...
        .show(
          html` <p>
              Je hebt 
              ${this._numberOk === 0 ? 'geen' : this._numberOk}
              ${this._numberOk === 1 ? "foto" : "foto's"} goed aanklikt in 1 minuut.
                </p>
              <p>Je hebt
              ${this._numberNok === 0 ? 'geen' : this._numberNok}
            ...
            if (result === 'again') 
                ...
                window.location.href = '/Rekenspelletjes/index.html';
        });
    ...
    render(){
        return html`
            <progress-bar style="--progress-bar-gameTime: 60s;" id="progressBar" @timeUp="${() => this.handleTimeUp()}"></progress-bar>
            <score-box id="scoreBox" 
...
customElements.define('click-correct-photo-on-numberline-app', ClickTheRightPhotoOnNumberLineApp);
